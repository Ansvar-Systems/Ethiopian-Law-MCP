#!/usr/bin/env tsx
/**
 * Ethiopia Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawethiopia.com. Ethiopian proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawethiopia.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'ethiopian-law-mcp/1.0 (https://github.com/Ansvar-Systems/ethiopian-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Ethiopia Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawethiopia.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawethiopia.com (Federal Negarit Gazette PDFs)`);
