#!/usr/bin/env tsx
/**
 * Ethiopian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawethiopia.com';
const PORTAL_NAME = 'Ethiopia Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/ethiopian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Ethiopian Law MCP — Data Freshness Check');
