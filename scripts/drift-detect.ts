#!/usr/bin/env tsx
/**
 * Drift detection for Ethiopia Law MCP.
 *
 * Checks if upstream Ethiopia Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Ethiopia Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'ethiopian-law-mcp/1.0 drift-detect' },
      });
