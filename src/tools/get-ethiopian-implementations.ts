/**
 * get_ethiopian_implementations — Find Ethiopian statutes implementing a specific EU directive/regulation
...
export interface GetEthiopianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface EthiopianImplementationResult {
  document_id: string;
...
export async function getEthiopianImplementations(
  db: InstanceType<typeof Database>,
  input: GetEthiopianImplementationsInput,
): Promise<ToolResponse<EthiopianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as EthiopianImplementationResult[];
