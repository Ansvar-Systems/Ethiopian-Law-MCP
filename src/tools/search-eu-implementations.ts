/**
 * search_eu_implementations — Search EU directives/regulations that have Ethiopian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_ethiopian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  ethiopian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as ethiopian_statute_count
  ...
  if (input.has_ethiopian_implementation) {
    sql += ' HAVING ethiopian_statute_count > 0';
  }
