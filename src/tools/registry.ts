/**
 * Tool registry for Ethiopia Law MCP Server.
...
import { getEthiopianImplementations, type GetEthiopianImplementationsInput } from './get-ethiopian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Ethiopia Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Ethiopian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Ethiopian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Ethiopian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Ethiopia?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Ethiopian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Ethiopian statute aligns with or references. ' +
      'Ethiopia is not an EU member but several Ethiopian laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Ethiopian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_ethiopian_implementations',
    description:
      'Find all Ethiopian statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Ethiopian statutes. ' +
      'Note: Ethiopia implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Ethiopian implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_ethiopian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Ethiopian aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Ethiopian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Ethiopian statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_ethiopian_implementations':
          result = await getEthiopianImplementations(db, args as unknown as GetEthiopianImplementationsInput);
          break;
