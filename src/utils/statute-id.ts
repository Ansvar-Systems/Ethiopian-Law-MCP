/**
 * Statute ID resolution for Ethiopia Law MCP.
 *
...
/**
 * Normalise "Act YYYY" to "Act, YYYY" — Ethiopia statutes store the comma form.
...
export function resolveDocumentId(
  ...
  if (chapMatch) {
    const chapNum = chapMatch[2];
    const lookup = getChapterLookup(db);
    const chapResult = lookup.get(chapNum);
