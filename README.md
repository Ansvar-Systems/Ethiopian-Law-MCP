...
An MCP server for Ethiopian legislation, maintained by [Ansvar Systems](https://ansvar.eu).
...
- Negarit Gazette via fdre.gov.et — `https://www.fdre.gov.et/`
- AfricanLII Ethiopia — `https://africanlii.org/akn/et/`
...
```bash
npm install -g @ansvar/ethiopian-law-mcp
```

This installs an MCP server that registers the standard Ethiopia Law tool surface (`search_legislation`, `get_provision`, etc.) but returns zero results from every query while the corpus is empty.

If your client expects non-empty results during this period, configure it to skip `eu.ansvar/ethiopian-law-mcp` until coverage is restored.
...
```bash
git clone https://github.com/Ansvar-Systems/Ethiopian-Law-MCP.git
